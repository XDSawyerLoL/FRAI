/*
  Assistant FRAI — Cloudflare Worker v6.7 propre Cloudflare
...
const WORKER_MODE = "v6.7-cloudflare-pages-worker-clean";
...
export default {
  async fetch(request, env) {
    ...
    if (path !== "/api/agent") {
      // Mode Cloudflare Pages : laisser les fichiers statiques passer vers l'asset binding.
      // Mode Worker seul : répondre en JSON pour confirmer que le Worker est actif.
      const isApiOrDebug = path === "/health" || path.startsWith("/debug") || path.startsWith("/api/");
      if (!isApiOrDebug && env.ASSETS && typeof env.ASSETS.fetch === "function") {
...
function buildOfficialLinks(intent, apiContext) {
  ...
  if (["recherche_emploi", "ues", "candidature_cv", "poe", "international"].includes(intent)) {
    // Les offres concrètes sont déjà renvoyées dans `offres` et affichées en cartes.
    // Ici on garde uniquement la logique d'action : une action principale + une piste secondaire.
    links.push(serviceToLink(SERVICE_CATALOG.bonneBoite));
    links.push(serviceToLink(SERVICE_CATALOG.events));
    return dedupeLinks(links).slice(0, 2);
  }
