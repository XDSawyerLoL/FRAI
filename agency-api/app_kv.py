...
def reply(data, status=200, payload=None):
    if iframe_transport(payload):
        ...
        body = '<!doctype html><meta charset="utf-8"><script>try{parent.postMessage(' + safe + ',"*")}catch(e){}<\/script>'
...
def events_js():
    ...
    payload = {'generatedAt': datetime.now(timezone.utc).isoformat(), 'count': len(items), 'events': items}
