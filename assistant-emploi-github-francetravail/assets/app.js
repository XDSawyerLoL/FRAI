...
const STORAGE_KEY = 'assistant_emploi_francetravail_github_v2';
...
function setStatus() {
  const city = state.settings.defaultCity ? ` · ville : ${state.settings.defaultCity}` : '';
  const ev = state.events.length ? `${state.events.length} événement(s)` : 'événements à importer';
  const lbi = state.lbi.length ? `${state.lbi.length} fiche(s) La Bonne Info` : 'La Bonne Info à importer';
  els.status.textContent = `Base locale · ${ev} · ${lbi}${city}`;
}
...
function eventCards(events) {
  return `<div class="cards">${events.map(e => {
    ...
    const meta = [
      `<span class="pill primary">${escapeHtml(formatDate(e.date, e.heure))}</span>`,
      e.ville ? `<span class="pill">${escapeHtml(e.ville)}</span>` : '',
      e.lieu ? `<span class="pill">${escapeHtml(e.lieu)}</span>` : '',
      e.type ? `<span class="pill yellow">${escapeHtml(e.type)}</span>` : '',
      e.modalite ? `<span class="pill green">${escapeHtml(e.modalite)}</span>` : ''
    ].filter(Boolean).join('');
    const desc = e.description ? `<p class="card-desc">${escapeHtml(e.description)}</p>` : '';
    const secteur = e.secteur ? `<p class="card-desc"><strong>Secteur :</strong> ${escapeHtml(e.secteur)}</p>` : '';
    const link = e.lien ? `<a class="primary" target="_blank" rel="noopener" href="${escapeHtml(e.lien)}">Voir la fiche / s’inscrire</a>` : '';
    return `<article class="event-card"><div class="card-top"><h3>${title}</h3><div class="meta">${meta}</div>${secteur}${desc}</div><div class="card-actions">${link}<a target="_blank" rel="noopener" href="${officialLinks.mee}">Ouvrir Mes Événements Emploi</a></div></article>`;
  }).join('')}</div>`;
}
...
function emptyEvents(city) {
  ...
  return `<div class="empty-state"><p><strong>Aucun événement importé ou trouvé${cityLabel}.</strong></p><p>Je n’affiche pas de faux résultat. Pour obtenir une réponse crédible en agence, il faut importer un export CSV/JSON ou brancher une API intranet autorisée.</p><p>En attendant, l’usager peut consulter la source officielle.</p><p><a class="inline-action" target="_blank" rel="noopener" href="${officialLinks.mee}">Ouvrir Mes Événements Emploi</a></p></div>`;
}

function emptyLbi() {
  return `<div class="empty-state"><p><strong>La base La Bonne Info n’est pas encore chargée.</strong></p><p>Le site intranet n’est pas lisible automatiquement depuis GitHub Pages. Importe un fichier JSON/Markdown ou configure une API intranet dans les réglages.</p><p>Format JSON attendu : <code>[{"titre":"...","categorie":"...","contenu":"...","motsCles":["..."]}]</code></p></div>`;
}
...
async function importEventsFile(file) {
  ...
  saveStored();
  addMessage('assistant', `<p><strong>${events.length} événement(s) importé(s).</strong></p><p>Tu peux maintenant demander : événements emploi dans ma ville.</p>`, { html: true });
}
...
async function importLbiFile(file) {
  ...
  saveStored();
  addMessage('assistant', `<p><strong>${items.length} fiche(s) La Bonne Info importée(s).</strong></p><p>Tu peux maintenant demander : cherche dans La Bonne Info actualisation, allocation, formation, etc.</p>`, { html: true });
}
...
function welcome() {
  addMessage('assistant', `<p>Bonjour. Je peux orienter sur les ateliers, prestations, PMSMP, POE, La Bonne Boîte, La Bonne Alternance, Mes Événements Emploi et La Bonne Info.</p><p>Pour les événements, je n’affiche que des données importées ou issues d’une API configurée. Aucun faux événement ne sera inventé.</p>`, { html: true });
}
