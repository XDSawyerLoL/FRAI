...
Ce paquet doit être envoyé **à la racine du dépôt** : `index.html`, `assets/`, `data/`, `.github/`, `manifest.webmanifest`, `sw.js` et `404.html` doivent être visibles directement au premier niveau du dépôt.

Ne publiez pas le dossier parent complet comme `assistant-france-travail-github/index.html`, sinon GitHub Pages cherchera `/index.html` à la racine et renverra une erreur 404.

Si une ancienne version a été ouverte dans le navigateur, videz le cache du site ou désinstallez l’application PWA avant de tester cette version, car un ancien service worker peut encore servir les anciens fichiers.
